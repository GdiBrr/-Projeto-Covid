...
conexao_banco = banco()
conexao_banco.inserir_dado()
# Data Generation
# Leitura dos dados de um arquivo CSV
#df = pd.read_csv("HIST_PAINEL_COVIDBR_13mai2021.csv", sep=";")
# Filtragem dos dados para obter os dados dos estados e do Brasil separadamente
#df_states = df[(~df["estado"].isna()) & (df["codmun"].isna())]
#df_brasil = df[df["regiao"] == "Brasil"]
# Salvando os dados filtrados em arquivos CSV para uso futuro
#df_states.to_csv("df_states.csv")
#df_brasil.to_csv("df_brasil.csv")

# Data Load
# Carregando os dados dos estados e do Brasil de arquivos CSV
df_states = conexao_banco.get_dados_estados()
df_brasil = conexao_banco.get_dados_brasil()

# Carregando o token do Mapbox de um arquivo de texto
token = open(".mapbox_token").read()
# Carregando os dados geojson dos estados do Brasil de um arquivo JSON
brazil_states = json.load(open("geojson/brazil_geo.json", "r"))

# Definição das chaves do dicionário contendo os dados dos estados
brazil_states["features"][0].keys()

# Selecionando os dados de uma data específica (2020-05-13) para uso posterior
df_states_ = df_states[df_states["data"] == "2020-05-13"]
# Mapeamento dos nomes das colunas para uma forma mais legível
...
# Criando um gráfico de mapa de calor usando Plotly Express
fig = px.choropleth_mapbox(df_states_, locations="estado",
    geojson=brazil_states, center={"lat": -16.95, "lon": -47.78},  # Coordenadas do centro do Brasil
...
fig.update_layout(
                # mapbox_accesstoken=token,  # Token de acesso ao Mapbox (comentado devido à não utilização)
                paper_bgcolor="#242424",  # Cor de fundo do papel
                mapbox_style="carto-darkmatter",  # Estilo do mapa do Mapbox
                autosize=True,
                margin=go.layout.Margin(l=0, r=0, t=0, b=0),
                showlegend=False,)  # Ocultando a legenda do gráfico

# Selecionando os dados de um estado específico (Rondônia) para uso posterior
df_data = df_states[df_states["estado"] == "RO"]

# Criando um gráfico de linha usando Plotly Graph Objects
fig2 = go.Figure(layout={"template":"plotly_dark"})
fig2.add_trace(go.Scatter(x=df_data["data"], y=df_data["casosAcumulado"]))
# Atualizando o layout do gráfico
fig2.update_layout(
    paper_bgcolor="#242424",  # Cor de fundo do papel
    plot_bgcolor="#242424",  # Cor de fundo do gráfico
    autosize=True,
...
def display_status(date, location):
    # Atualizando as informações exibidas com base na data e localidade selecionadas
    if location == "BRASIL":
...
def plot_line_graph(plot_type, location):
    # Plotando o gráfico de linha com base no tipo de dado selecionado e na localidade escolhida
    if location == "BRASIL":
...
def update_map(date):
    # Atualizando o mapa de cloroplasma com base na data selecionada
    ...
    fig = px.choropleth_mapbox(df_data_on_states, locations="estado", geojson=brazil_states, 
        center={"lat": CENTER_LAT, "lon": CENTER_LON},  # Coordenadas do centro do Brasil
...
def update_location(click_data, n_clicks):
    # Atualizando o botão de localização com base nos dados de clique ou no número de cliques
...
if __name__ == "__main__":
    app.run_server(debug=False, port=8051)  # Executando o servidor do Dash
